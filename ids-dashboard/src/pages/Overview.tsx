...
const Overview: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const networkDatas = await fetchNetworkData(900);
    console.log('Network data structure:', networkData);
    if (networkData && networkData.length > 0) {
      console.log('First network data item:', networkData[0]);
    }
  ...
  const formatNetworkData = () => {
    ...
    // Get the most recent network data
    const latestData = networkData[networkData.length - 1];
    
    // Check if the expected properties exist, use fallbacks if not
    return {
      inboundTraffic: latestData.incoming_mbps !== undefined ? 
        Math.round(latestData.incoming_mbps * 100) / 100 : 
        (latestData.incoming_bytes ? Math.round(latestData.incoming_bytes / (1024 * 1024) * 100) / 100 : 0),
        
      outboundTraffic: latestData.outgoing_kbps !== undefined ? 
        Math.round(latestData.outgoing_kbps * 100) / 100 : 
        (latestData.outgoing_bytes ? Math.round(latestData.outgoing_bytes / 1024 * 100) / 100 : 0),
        
      totalInbound: latestData.total_incoming_gb !== undefined ? 
        Math.round(latestData.total_incoming_gb * 100) / 100 : 
        (latestData.total_incoming_bytes ? Math.round(latestData.total_incoming_bytes / (1024 * 1024 * 1024) * 100) / 100 : 0),
        
      totalOutbound: latestData.total_outgoing_mb !== undefined ? 
        Math.round(latestData.total_outgoing_mb * 100) / 100 : 
        (latestData.total_outgoing_bytes ? Math.round(latestData.total_outgoing_bytes / (1024 * 1024) * 100) / 100 : 0),
        
      packetLoss: latestData.packet_loss_mb !== undefined ? 
        Math.round(latestData.packet_loss_mb * 100) / 100 : 
        (latestData.packet_loss_bytes ? Math.round(latestData.packet_loss_bytes / (1024 * 1024) * 100) / 100 : 0)
    };
  };

  const networkMetrics = formatNetworkData();

  const { inboundTraffic, outboundTraffic, totalInbound, totalOutbound, packetLoss } = networkMetrics;
        ...
        >
          ...
          <div className="flex flex-col justify-center items-center h-40">
            <div className="text-center">
              ...
              <div className="text-3xl font-bold text-gray-100">{inboundTraffic}KB/s</div>
